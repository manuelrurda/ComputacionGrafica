...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	{
		//Rotacion cofre
		if (theWindow->articulacion1 + hoodRotation > 60 || theWindow->articulacion1 + hoodRotation < 0) {
			hoodRotation *= -1;
		}
		theWindow->articulacion1 += hoodRotation;
	}

	if (key == GLFW_KEY_G)
	{
		//Rotacion llantas para adelante
		theWindow->articulacion2 += 20.0;
		theWindow->traslacion += 5.0f;
	}
	if (key == GLFW_KEY_H)
	{
		//Rotacion llantas para atras
